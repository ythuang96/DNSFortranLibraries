...
#   include "parameters"
    ...
    complex(C_FLOAT_COMPLEX), dimension(mgalx) :: vector_ifftx
    complex(C_FLOAT_COMPLEX), dimension(mgalz) :: vector_ifftz
    ...
    complex(C_FLOAT_COMPLEX), dimension(nt) :: vector_tom_c2c
...
contains
    ...
    subroutine fft_plan
        ...
        plan_ifftx = fftwf_plan_dft_1d(mgalx, vector_ifftx,vector_ifftx, FFTW_BACKWARD, FFTW_PATIENT )
        plan_ifftz = fftwf_plan_dft_1d(mgalz, vector_ifftz,vector_ifftz, FFTW_BACKWARD, FFTW_PATIENT )
        ...
        plan_fftt_c2c = fftwf_plan_dft_1d( nt, vector_tom_c2c, vector_tom_c2c, FFTW_BACKWARD, FFTW_PATIENT)
    ...
    subroutine fft2( matrix_in, matrix_out )
        ...
        call fftwf_execute_dft_r2c(plan_fft2, matrix_2d_real, matrix_2d_comp)
    ...
    subroutine ifft2( matrix_in, matrix_out )
        ...
        call fftwf_execute_dft_c2r(plan_ifft2, matrix_2d_comp, matrix_2d_real)
    ...
    subroutine ifftx( matrix_in, matrix_out )
        ...
        DO ii = 1, nkz_pos
            ...
            call fftwf_execute_dft(plan_ifftx, vector_ifftx, vector_ifftx )
    ...
    subroutine ifftz( matrix_in, matrix_out )
        ...
        DO ii = 1, nkx_pos
            ...
            call fftwf_execute_dft(plan_ifftz, vector_ifftz, vector_ifftz )
            ...
            DO ii = 1, size_dim1
                DO kk = 1,nt
                    if (WindowFunction .eq. "None" ) then ! No window function
                        ...
                        vector_t(kk) = matrix_in(ii,jj,kk) &
                        * real((0.54_dp - 0.46_dp * cos(2.0_dp * pi * real(kk-1,dp)/real(nt-1,dp))),sp)
                    else if (WindowFunction .eq. "Hann" ) then ! Hann function
                        vector_t(kk) = matrix_in(ii,jj,kk) &
                        * real((0.50_dp - 0.50_dp * cos(2.0_dp * pi * real(kk-1,dp)/real(nt-1,dp))),sp)
                    endif
                ENDDO

                ! perform transform
                call fftwf_execute_dft_r2c( plan_fftt, vector_t, vector_om)

                DO kk = 1,nt/2+1
                    matrix_out(ii,jj,kk) = conjg( vector_om(kk) )/real(nt,sp)
    ...
    !
    ! Arguments
    !   matrix: [single real, size (:,:,nt), Input/Output]
    !           Time domain data as the input. The third dimension needs to be time
    ...
    subroutine fftt_c2c( matrix )
        ...
        DO jj = 1, size_dim2
            DO ii = 1, size_dim1
                ...
                ! perform transform
                call fftwf_execute_dft( plan_fftt_c2c, vector_tom_c2c, vector_tom_c2c )
                ...
                matrix(ii,jj,:) = vector_tom_c2c(:) /real(nt,sp)
            ENDDO
        ENDDO
